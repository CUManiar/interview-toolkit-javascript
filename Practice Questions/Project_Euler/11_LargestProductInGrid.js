...
function findLargestProductInGrid(grid) {
    let matrix = [];
    let maxProduct = 0;

    var arr = grid.split(' ').map(i => parseInt(i)); // Convert into array from string
    for(let i = 0; i < arr.length; i+=20) {
        matrix.push(arr.slice(i, i+20));  // Create matrix of 20*20
    };
    ...
    for (var i = 0; i < matrix.length; i++) {
        ...
        for (var j = 0; j < row.length; j++) {
            ...
            if (j < row.length - 3) {
                ...
                values = [];
                    values.push([row[j], row[j + 1], row[j + 2], row[j + 3]])
                    maxProduct = product;
                }
            }
            ...
            if (i < matrix.length - 3) {
                ...
                if (product > maxProduct) {
                values = [];
                  values.push([matrix[i][j], matrix[i + 1][j], matrix[i + 2][j], matrix[i 	+ 3][j]])
                    maxProduct = product;
                }
            }
            ...
            if ((j < row.length - 3) && (i < matrix.length - 3)) {
                ...
                if (product > maxProduct) {
                values = [];
                    values.push([matrix[i][j], matrix[i + 1][j + 1], matrix[i + 2][j + 2], matrix[i + 3][j + 3]])
                    maxProduct = product;
                }
            }
